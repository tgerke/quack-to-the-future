...
The discipline is real. [The tooling never showed up.]{.orange}
:::

::: {.fragment}
::: {.statement style="font-size: 1.05em; margin-top: .6em;"}
Data pipelines need a **paper trail**:

what changed, *who* changed it, *why*, and where every column **came from**.
:::
...
::: notes
Everything is visible on arrival except the stem; one click brings that in. Left column is the room's reflex: git, renv, CI on every push. Right column names the real practice with respect: ISO-dated folders and suffixes are discipline, but there's no author, no diff, no rollback. The gap was never carelessness; the tooling never showed up for data. Then the stem, slowly: everything in the next 18 minutes hangs off this sentence. What changed, who, why is chapters one and two; where every column came from is chapter three.
:::
