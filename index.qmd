...
::: notes
Two clicks. The slide opens on March: a dated folder for each data drop, a script in git that reads one of them, and the number it printed. Most of the room runs this setup. First click, June. Someone opens the spreadsheet, fixes a cell, and saves. Same code, git shows no change, and the number is different. The only trace is a modified date three months after the folder's, and the photo starts to fade. Second click, September. A colleague asks why the number changed since March, and you have no idea. Nothing says who or why. Filenames are our version control, Slack is our audit log, memory is our lineage system. The question is Marty's alternate 1985: someone changed the data upstream and you can't see where the timeline forked. Pays off in chapter two. Biff's small tweak is this June edit made inside a lake, where there is no such thing as an in-place edit: every change lands as a new snapshot with an author and a message, and the old rows stay readable. The spreadsheet itself is what bronze is for: load it as received under an authored commit, and when an edited copy shows up later, the difference between the two loads is a query.
:::
