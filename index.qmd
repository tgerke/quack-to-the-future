...
::: notes
"Hi, I'm Travis. I lead applied intelligence at the Prostate Cancer Clinical Trials Consortium, and this talk is about time travel for your data."
:::
...
We've had the discipline for years, [just not the tool]{.orange}.
:::

::: {.quiet style="text-align: center; margin-top: .8em;"}
In this talk, we'll see how to produce:
:::

::: {.statement style="font-size: 1.05em; margin-top: 0;"}
**Git-grade history** for your data, *without leaving R*.
:::
:::

::: notes
One click: the two columns land first, then the whole bottom block arrives together. Left column is the room's reflex: git, renv, CI on every push. Right column names the real practice with respect: ISO-dated folders and suffixes are discipline, but there's no author, no diff, no rollback. "More than one way" does the tool-agnostic work early; the next slide names Iceberg and Delta, and the close returns to it. Land on the last line and say the stem out loud: data pipelines need a paper trail, what changed, who changed it, why, and where every column came from. That sentence is the spine of the talk: what, who, why is chapters one and two; where every column came from is chapter three; and "paper trail" pays off at the Part III turn and the summary slide.
:::
...
[This session is called "From laptop to cluster." Everything you're about to see runs on the laptop.]{.quiet}
:::
:::
:::

::: notes
Audience calibration slide: most of the room has heard "lakehouse" but wouldn't define it. The recipe lands as one block; the single click brings the catch. If pressed on the bullets: the file format is technically pluggable (Iceberg also takes ORC and Avro) but Parquet is the near-universal choice, and every flavor has a catalog in function if not in name. Delta keeps a transaction log beside the data, Iceberg uses metadata files plus a catalog service, and DuckLake uses a plain SQL database, which is the punchline two slides ahead. "Spark clusters" stands in for the JVM stack nobody in this room runs. The laptop aside is real: this talk sits in a session called "From laptop to cluster," and everything in it runs locally; DuckDB reaches for a cluster only when the data outgrows one machine. Setup ends here: the pattern exists, this room was priced out of it, and the next three chapters are the door.
:::
...
That's it. No cluster, no platform team, no Zookeeper sacrifice.
