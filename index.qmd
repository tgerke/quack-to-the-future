...
::: notes
Medallion in 20 seconds: it's just staged refinement with rules. The demo follows these three hops, and each layer gets a schema of its own, so the folder on the DuckLake slide is the medallion.
:::
...
```{r}
#| message: true
gas_guzzlers <- get_ducklake_table("silver.vehicles") |>
  filter(mpg < 25) |>          # "cleanup"
  select(model)

with_transaction(
  rows_delete(get_ducklake_table("silver.vehicles"), gas_guzzlers, by = "model"),
...
```{r}
#| include: false
# later slides print snapshot numbers as literals; fail the render if they drift
stopifnot(max(list_table_snapshots()$snapshot_id) == 5)
```

32 rows → 6 rows, on a Friday afternoon, labeled **"small tweak."**

::: notes
Every team has a Biff. Sometimes Biff is you, at 4:47 PM. You can't prevent the mistake; you can make it visible, attributed, and reversible. Delivery note for me: row-level operations (rows_delete, rows_update) are what the change feed on the next slide traces; a full replace_table() rewrite is a drop-and-create, so its history starts over. That's why Biff's tweak is a rows_delete.
:::

## "What happened?" is now a query

```{r}
get_table_changes("silver.vehicles", start = 5, end = 5) |>
...
::: notes
The payoff of the changed-number question from the hook. DuckLake calls this the data change feed: every insert, delete, and update pre/post image between any two snapshots, as a lazy table you filter with dplyr. Snapshot 5 is the number Biff's confirmation printed on the last slide.
:::
...
::: notes
The money demo. For one pipeline it is just `gold |> extract_lineage() |> lineage_flow()`; the named list stitches the layers so bronze, silver, and gold appear as one graph. Next slide is the live widget: drag a table, hover a column to highlight its connections and see the expression label (as.integer(cyl) into silver, mean(mpg) into gold), click a column to isolate everything upstream and downstream of it.
