...
```{r}
#| message: true
df_vehicles <- mtcars |> tibble::rownames_to_column("model")
...
::: notes
The raw data is mtcars, with the car names moved out of the row names into a model column: a database keeps no row names, and that column is the key everything later hangs on. It lands in bronze, as received, one commit. Every commit carries an author and a message, and the confirmation names the snapshot. That snapshot number is the version number we'll travel to later. Snapshot 0 is the lake's creation.
:::
