...
We version our **code** religiously.

The data it produces? Every overwrite quietly **erases history**:
...
[Version the data like you version the code.]{.orange}
:::
:::
:::

::: notes
The opener, straight off the title slide. Let the crisp photo sit while you name the double standard: years of git, renv, and CI for code, overwrite-and-hope for data. Click: the photo fades. Every `write_csv()` over an existing file is Marty's siblings disappearing: the present looks fine, and the past is just gone. Click: the ask, out loud in minute one. The next two slides are what the fade feels like from the inside.
:::
...
Somewhere in the past, your data's timeline **skewed**.
:::

::: notes
The fade, lived: a problem everyone in the room has had. Let the question hang. Almost every hand goes up if you ask who's been here.

Last click is the Part II beat: Marty comes back to a 1985 gone wrong and can't tell where the timeline forked. Say it plainly: "Someone changed your data somewhere upstream, and you're living in the alternate 1985 of your own report." Pays off later at the almanac/lineage slide.
:::
...
For **code**, this room treats version control as a given:

::: incremental
- every change is a commit, with an author and a message
- any past state is one checkout away
- "who changed this, and why?" has a command: `git blame`
:::

::: {.fragment style="text-align: center; margin-top: .6em;"}
The data your pipelines produce gets none of that.

[Overwrite and hope.]{.orange}
:::

::: notes
The hook named the double standard; this slide makes it mechanical. Commits, checkouts, and blame on one side; the pipeline's output landing in a CSV with no history on the other. Nobody chose that on purpose; the tooling made one easy and the other hard.
:::
...
That's the whole talk: git-grade history for your data, without leaving R.
:::
