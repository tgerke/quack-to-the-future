...
<div class="term-bar"><span class="dot r"></span><span class="dot y"></span><span class="dot g"></span> analysis.R &nbsp;·&nbsp; git: unchanged since Mar 14</div>
...
2026-03-14/
  ...
  export_v2_FINAL.xlsx  <span class="hl">Jun  2  ← edited in place</span></div>
</div>
```
:::
...
::: notes
Most of the room runs this setup: a dated folder for each data drop, a script in git that reads one of them. Then someone opens the spreadsheet, fixes a cell, and saves. Git sees nothing. It only watches the code. In September a colleague asks why a number changed since March, and you have no idea. The folder says March 14, the file says June 2, and nothing says who or why. Filenames are our version control, Slack is our audit log, memory is our lineage system. The question is Marty's alternate 1985: someone changed the data upstream and you can't see where the timeline forked. Pays off in chapter two. Biff's small tweak is this June 2 edit made inside a lake, where there is no such thing as an in-place edit: every change lands as a new snapshot with an author and a message, and the old rows stay readable. The spreadsheet itself is what bronze is for: load it as received under an authored commit, and when an edited copy shows up later, the difference between the two loads is a query.
:::
...
::: notes
Medallion in 20 seconds: it's just staged refinement with rules, and the demo follows these three hops. Bronze is also the answer to the opening's edited spreadsheet: load the file as received, under an authored commit, and the lake's copy is the record of what arrived and when. Schema is the word the code uses from here on: create_schema("bronze") makes the layer, and every table name carries it, as in bronze.vehicles. The folder listing on the DuckLake slide is these three schemas on disk.
:::
